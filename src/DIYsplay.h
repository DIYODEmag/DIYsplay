/*
 * DIYsplay.h
 * Author: Liam Davies (l.davies@diyodemag.com)
 * Copyright (c) 2022 DIYODE Magazine
 */

#ifndef _DIYSPLAY_H_
#define _DIYSPLAY_H_

// Ensure includes directory is okay
#include "MatesController.h"
#include "SoftwareSerial.h"

#define DEFAULT_SIG_PIN 3
#define DEFAULT_RESET_PIN 4

// Please UNCOMMENT this line if you wish to use AltSoftSerial.
//#include "AltSoftSerial.h"

/**
 * @brief 
 * This is an enumerated list that corresponds with the gauges programmed onto the DIYsplay.
 * The end-user enters the Enum directory which is implicitly casted to an Page number
 * corresponding with the correct Page on the display unit.
 * 
 * The only downside of this method is that the order is completely arbitrary and does not
 * hold up to inserting displays in the middle of the array. These bindings are generated by a
 * Python script that automatically gathers data from a MATES firmware file.
 */
typedef enum
{
     DIYSPLAY_LOGO,
     _5_GAUGE_EQ,
     BAR_METER_LEDDIGITS,
     LCD_BAR_METER,
     MEDIA_SPECTRUM_L_TO_R,
     MEDIA_SPECTRUM,
     METAL_BAR_METER_SINGLE,
     METAL_BAR_METER_SINGLE_2,
     SOUND_LEVEL_METER,
     DB_METER_LEFT_AND_RIGHT_BLUE,
     DB_METER_LEFT_AND_RIGHT_DUAL,
     DB_METER_LEFT_AND_RIGHT,
     DB_METER_SINGLE_LED_BAR,
     DB_METER_AVG_AND_PEAK_LR,
     DIGITAL_CLOCK_LARGE,
     DIGITAL_CLOCK,
     LIDAR_DISTANCE_METER,
     RULER_GAUGE_SCALE,
     _2_DECIMAL_POINT_LEDDIGITS_X4_RED,
     _2_DECIMAL_POINT_LEDDIGITS_LARGE,
     _2_DIGIT_LEDDIGITS_X3_RED,
     AMMETER_A,
     AMMETER_BLACK,
     BATTERY_LEVEL_GAUGE_BLUE,
     BATTERY_LEVEL_GAUGE,
     DUAL_GAUGE_LEDDIGITS,
     METER_LED_GAUGE,
     VOLTMETER_BLACK,
     CIRCULAR_CROPPED_TO_HALF_MOON_LARGE_TEXT_COLD_HOT,
     TEMPERATURE_HUMIDITY_WITH_TEXT_AREAS,
     FULL_SCOPE,
     GAUGE_LEDDIGITS_X3,
     RGB_GAUGES,
     RULER_GAUGE_WITH_SCALE,
     VARIOUS_GAUGES_BLUE,
     VARIOUS_GAUGES_GREEN,
     VARIOUS_GAUGES_RED,
     _8DIGIT_LEDDIGITS_X2,
     CIRCULAR_CROPPED_TO_HALF_MOON_LARGE_TEXT_LEDDIGITS_RIGHT,
     CIRCULAR_CROPPED_TO_HALF_MOON_LARGE_TEXT,
     CIRCULAR_CROPPED_TO_HALF_MOON_SMALL_TEXT_LEDDIGITS,
     CROPPED_DUAL_0_TO100,
     CROPPED_DUAL_LOW_TO_HIGH,
     DUAL_LED_BAR_GAUGE,
     HALF_MOON_BEZEL_TOP,
     HALF_MOON_DUAL_GAUGES,
     HALF_MOON_DUAL_LEDDIGITS,
     HALF_MOON_LED_LEFT,
     LARGE_LEDDIGITS_RED,
     LEDDIGITS_X3,
     QUARTER_MOON_DUAL_BEZELS_TOP,
     QUARTER_MOON_DUAL_ORANGE,
     QUARTER_MOON_LEDDIGITS_GAUGE_ORANGE,
     QUARTER_MOON_ROUNDED_DUAL,
     QUARTER_MOON_SINGLE_ORANGE_LEDDIGITS_GAUGE_MULTIPANEL,
     VARIOUS_DIGITS,
     ANGLE_DISPLAY,
     MEDIA_PLAYER_RED,
     MULTIMEDIA_INDICATOR_GREEN_GRADIENT,
     ALTIMETER,
     COMPASS_SIMPLE,
     COMPASS_WITH_LONGITUDE_LATITUDE,
     RULER_GAUGE_TAPE_COMPASS,
     SCALE_TAPE_COMPASS,
     _3_X_COLOURED_TEXT_LINES,
     _3_X_PLAIN_TEXT_LINES,
     _3LINE_BLACK_BG_PRINT_AREA,
     _6_X_COLOURED_TEXT_LINES,
     _6_X_PLAIN_TEXT_LINES,
     _6LINE_BLACK_BG_PRINT_AREA,
     _8_X_FLAT_ROUND_LED,
     COLOUR_LED_WITH_GAUGES,
     WAVE_STAT_DISPLAY,
} Screen;

class DIYsplay
{
public:
     /**
      * @brief Construct a new DIYsplay object. Note that you must call the begin() function
      * on this object before any functionality can be used.
      *
      */
     DIYsplay();

     /**
      * @brief Begins communication with the DIYsplay. By default, this uses the inbuilt hardware
      * Serial line built into your Arduino.
      *
      */
     void begin();

     /**
      * @brief Starts up communication with the DIYsplay over a HardwareSerial connection.
      * Note that your Arduino board must have a hardware serial line for this to work.
      *
      * @param serial The HardwareSerial instance to use.
      * @param sigPin The GPIO pin to be to used for the DIYsplay's SIG pin.
      * @param resetPin The GPIO pin to be to used for the DIYsplay's RES (reset) pin.
      */
     void begin(HardwareSerial &serial,
                uint8_t sigPin = DEFAULT_SIG_PIN,
                uint8_t resetPin = DEFAULT_RESET_PIN);

// Only allow this constructor to be used if Software Serial is defined.
#ifdef SoftwareSerial_h
     /**
      * @brief Starts up communication with the DIYsplay over a SoftwareSerial connection.
      * Note that you need to have included the SoftwareSerial library for this to work.
      *
      * @param serial The SoftwareSerial instance to use.
      * @param sigPin The GPIO pin to be to used for the DIYsplay's SIG pin.
      * @param resetPin The GPIO pin to be to used for the DIYsplay's RES (reset) pin.
      */
     void begin(SoftwareSerial &serial,
                uint8_t sigPin = DEFAULT_SIG_PIN,
                uint8_t resetPin = DEFAULT_RESET_PIN);
#endif

// Only allow this constructor to be used if AltSoftSerial is defined.
//   Note that because it is NOT included by default in the Arduino IDE,
//   you must install it from the Library Manager and uncomment the define
//   line at the top of this file.
#ifdef AltSoftSerial_h
     /**
      * @brief Starts up communication with the DIYsplay over a AltSoftSerial connection.
      * Note that you need to have included the AltSoftSerial library for this to work.
      *
      * @param serial The AltSoftSerial instance to use.
      * @param sigPin The GPIO pin to be to used for the DIYsplay's SIG pin.
      * @param resetPin The GPIO pin to be to used for the DIYsplay's RES (reset) pin.
      */
     void begin(AltSoftSerial &serial,
                uint8_t sigPin = DEFAULT_SIG_PIN,
                uint8_t resetPin = DEFAULT_RESET_PIN);
#endif

     /**
      * @brief Set the currently displayed screen.
      * @param screen pass in an index number that corresponds to the desired screen.
      *
      */
     void setScreen(int screen);

     /**
      * @brief Set the currently displayed screen.
      * @param screen pass in a Screen enumerator that corresponds to the desired screen.
      *
      */
     void setScreen(Screen screen);

     // We're re-initializing this instance when the DIYsplay::begin() method
     // is called because Mates doesn't have a default constructor.
     // We set the correct parameters there.
     MatesController mates = MatesController(Serial);
     int currentScreen;

     /**
      * @brief Set the Data object
      * 
      * @param index 
      * @param data 
      */
     void setData(int16_t index, int16_t data);
     void setData(int16_t index, String data);

     /**
      * @brief Prints text onto a currently displayed PrintArea. This will NOT work
      * with TextAreas, so use setData() if it is currently displayed.
      * This will automatically format text as required.
      * @param text The text to be printed.
      */
     void printText(String text);

     /**
      * @brief Clear the current text displayed on the PrintArea. Note that
      * this will have no affect on TextArea screens.
      * 
      */
     void clearText();

     /**
      * @brief Gets the number of widgets on a requested screen.
      * @param screen The screen to find the number of widgets of. This is an optional parameter.
      * If not passed, this will return the number of widgets on the currently displayed screen.
      * @return The number of widgets on the screen object.
      */
     uint8_t getWidgetCount(Screen screen);
     /**
      * @brief Gets the number of widgets on a requested screen.
      * @param screen The screen to find the number of widgets of. This is an optional parameter.
      * If not passed, this will return the number of widgets on the currently displayed screen.
      * @return The number of widgets on the screen object.
      */
     uint8_t getWidgetCount(uint16_t screen);

     /**
      * @brief Gets the number of widgets on the current screen.
      *
      * @return The number of widgets on the current screen.
      */
     uint8_t getWidgetCount();

private:
     // Initializes interface with MatesController
     void init();

     uint8_t sigPin;
     uint8_t resetPin;

     /**
      * @brief This array stores the number of widgets on each DIYsplay screen.
      * This should be used with the getWidgetLength() command.
      *
      */
     const uint8_t widgetLengths[73] = {
         0, 10, 20, 10, 12, 12, 4,
         10, 2, 4, 12, 8, 6, 8,
         4, 7, 4, 2, 8, 2, 6,
         6, 4, 4, 4, 8, 6, 4,
         2, 10, 2, 12, 12, 2, 6,
         6, 6, 4, 4, 2, 4, 4,
         4, 4, 2, 8, 8, 4, 2,
         6, 4, 4, 6, 4, 8, 6,
         4, 29, 10, 5, 2, 4, 4,
         6, 3, 3, 0, 6, 6, 0,
         16, 14, 4};
};
#endif
;